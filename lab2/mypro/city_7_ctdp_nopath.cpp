...
void city_nokia::put_newstate(int i,int oristate,int der,int newval){/*{{{*/
	...
	if(calc_pro[i+1][newstate]<newval)
		calc_pro[i+1][newstate]=newval;
	return ;
}/*}}}*/
...
:rows(row),cols(col)
{
	...
	calc_pro = new int * [cols+1];
	for(i=0;i<=cols;i++)
		calc_pro[i]=new int [seven[cols]];
